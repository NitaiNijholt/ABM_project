...
def perform_ks_test_on_individual_runs(base_directory, given_csv_filename):
    ...
    given_csv_path = os.path.join(base_directory, given_csv_filename)
    ...
    plt.plot(np.sort(given_income_normalized), np.linspace(0, 1, len(given_income_normalized)), 
             label='Empirical Data CDF', color='black', linestyle='--')

    # Finalize CDF plot
    plt.title(f'CDF Comparison for All Runs vs. Empirical Data')
...
base_directory = './sensitivity_analysis_results'
given_csv_filename = '../data/cleaned_income_data.csv'
