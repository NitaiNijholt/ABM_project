class SimulationEvolve:
    ...
    def reproduce(self):
        ...
        for reproduction in range(int(self.num_agents / 2)):
            n_offspring = 2 if np.random.rand() <= 0.1 else 1
            mating_pool = np.array([self.tournament() for _ in range(2)])
            offspring = self.line_recombination([mating_pool[0].network, mating_pool[1].network], n_offspring)
    ...
    def run(self, show_time=False):
        epochs = 15
        self.show_time = show_time
        self.tax_policy = DynamicTaxPolicy(self.grid, self) if self.dynamic_tax else StaticTaxPolicy(self.grid, self)
    ...
    def get_agents_dict(self):
        return {agent_id: {'wealth': agent.wealth, 'wood': agent.wood, 'stone': agent.stone} for agent_id, agent in self.grid.agents.items()}
